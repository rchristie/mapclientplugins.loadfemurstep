...
def loadfemur(filenameIn, filenameOut):
    ...
    first = True
    for ni in range(firstTopNode, lastTopNode):
    ...
    plate_size = 2.5*vector_magnitude(node15pos_delta)
    const05 = fm.createFieldConstant([-0.5,-0.5,0.0])
    xi = fm.findFieldByName("xi")
    xiOffset = fm.createFieldAdd(xi, const05)
    constPlateScale = fm.createFieldConstant([plate_size, plate_size, plate_size])
    xiScaleOffset = fm.createFieldMultiply(xiOffset, constPlateScale)
    plateCentre = [(bottomCentre[i] + 0.2*bottomToTop[i]) for i in range(3)]
    constPlateCentre = fm.createFieldConstant(plateCentre)
    plateCoordinates = fm.createFieldAdd(xiScaleOffset, constPlateCentre)
...
# loadfemur("femur_lowerextremity.exfile", "femur_result.exfile")
